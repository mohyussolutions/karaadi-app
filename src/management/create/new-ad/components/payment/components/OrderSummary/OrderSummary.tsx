...
import { useDisplayAttrs } from '../../../../../../../hooks/useDisplayAttrs';
...
import { CheckoutHeader } from '../Checkout/Checkout';
import { createStyles } from '../../../../../../../util/styles/payment/orderSummary.styles';
import { bgColor, textColor } from '../../../../../../../util/styles/common/dynamic.styles';
...
const PriceBreakdown = memo(function PriceBreakdown({ plan, ps, feeAmount }: PriceBreakdownProps) {
  ...
  return (
    <View style={s.breakdown}>
      ...
      <View style={s.bRow}>
        <View style={[s.planBadge, bgColor(ps.bg)]}>
          <MaterialCommunityIcons name={ps.icon} size={12} color={ps.color} />
          <Text style={[s.bKey, textColor(ps.color)]}>{plan.label}</Text>
  ...
  return (
    <View style={[s.totalBox, isFree ? s.totalBoxFree : s.totalBoxPaid]}>
      <View style={s.totalLeft}>
        <MaterialCommunityIcons name="receipt-text-check-outline" size={16} color={isFree ? Colors.successDark : Colors.primary} />
        <Text style={[s.totalLabel, isFree ? s.green : s.totalLabelPaid]}>{t('postAd.totalDue')}</Text>
      </View>
      <Text style={[s.totalAmt, isFree ? s.green : s.totalAmtPaid]}>
...
export function OrderSummary({ plan, item, categoryName, feeAmount }: OrderSummaryProps) {
  ...
  const displayAttrs = useDisplayAttrs(item);

  return (
    <View style={s.wrap}>
      <CheckoutHeader
        icon="receipt-text-outline"
        title={t('postAd.orderSummary', { defaultValue: 'Order Summary' })}
        subtitle={t('postAd.reviewBeforePay', { defaultValue: 'Review your listing before paying' })}
      />
      {images.length > 0 ? <ImageCarousel images={images} index={imgIdx} onChangeIndex={setImgIdx} /> : <EmptyImageState />}
