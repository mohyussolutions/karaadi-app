...
export async function updatePhoneVisibility(hidePhone: boolean): Promise<User> {
  const { data } = await apiClient.put(AUTH_ENDPOINTS.UPDATE_PHONE_VISIBILITY, { hidePhone });
  return data.user ?? data;
}
