...
async function buildHeaders(extra?: ExtraHeaders, isFormData?: boolean): Promise<Record<string, string>> {
...
async function handle401() {
  ...
  const { disconnectSocket } = require('./sockets/socket.actions') as typeof import('./sockets/socket.actions');
  disconnectSocket();
  const { store } = require('../store') as typeof import('../store');
  const { clearCredentials } = require('../store/slices/authSlice') as typeof import('../store/slices/authSlice');
...
async function request<T>(
  ...
  if (!res.ok) {
    let errData: unknown;
    try { errData = await res.json(); } catch { errData = await res.text().catch(() => ''); }
    throw Object.assign(new Error(`HTTP ${res.status}`), { response: { status: res.status, data: errData } });
  }
...
export type RequestOptions = { params?: Params; headers?: ExtraHeaders; signal?: AbortSignal };
